...
- Cost block fixed semantics: cost_pp = cost_per_unit * multiplier (per_sqft/per_bdft check member.units_per_product override)
- Phase 2 block architecture: quote_blocks + quote_block_members replace per-product blocks and group pools
- Margin rates: per-block default (block.margin_rate) with per-product override (member.margin_rate). Old per-category product fields removed.
- system_defaults → quote defaults → product: inheritance chain for hourly_rate
...
6. Pipelines (species, stone, stock base, rate labor) must not overwrite user-customized rates — only create blocks if missing.
7. System defaults → quote defaults → product defaults: inheritance for hourly_rate. Margin rates live on blocks/members, not products.
