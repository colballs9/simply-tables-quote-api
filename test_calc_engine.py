...
class TestGroupCostPool:
    ...
    def test_distribute_by_sqft_uses_sq_ft_wl_for_dia(self):
        ...
        pool = {"total_amount": 500, "distribution_type": "sqft"}
        members = [
...
class TestFarmhouseKitchen0737:
    ...
    def _build_quote(self):
        ...
        def product(pid, qty, w, l, shape, base_type, extra_blocks=None):
            blocks = [
                # Species cost: $9.50/bdft (per_bdft multiplier)
                {"id": f"{pid}_sp", "cost_category": "species",
                 "cost_per_unit": "9.50", "multiplier_type": "per_bdft",
                 "units_per_product": 1, "tag_id": None},
            ]
            if extra_blocks:
                blocks.extend(extra_blocks)
            return {
                "id": pid, "quantity": qty, "width": w, "length": l,
                "shape": shape, "shape_custom": "Booth Table" if shape == "Custom Shape" else None,
                "material_type": "Hardwood", "lumber_thickness": '1.75"',
                "base_type": base_type, "bases_per_top": 2 if pid == "p4" else 1,
        ...
        products = [
            product("p1", 4, 30, 46, "Custom Shape", "Stock Base", [
                {"id": "p1_sb", "cost_category": "stock_base",
                 "cost_per_unit": 75, "multiplier_type": "per_base",
                 "units_per_product": 1, "tag_id": None},
            ]),
            product("p2", 1, 60, 60, "DIA", "Stock Base", [
                {"id": "p2_sb", "cost_category": "stock_base",
                 "cost_per_unit": 160, "multiplier_type": "per_base",
                 "units_per_product": 1, "tag_id": None},
            ]),
            product("p3", 4, 36, 36, "Standard", "Stock Base", [
                {"id": "p3_sb", "cost_category": "stock_base",
                 "cost_per_unit": 45, "multiplier_type": "per_base",
                 "units_per_product": 1, "tag_id": None},
            ]),
            product("p4", 2, 30, 48, "Standard", "Stock Base", [
                # 2 bases_per_top at $35 each
                {"id": "p4_sb", "cost_category": "stock_base",
                 "cost_per_unit": 35, "multiplier_type": "per_base",
                 "units_per_product": 1, "tag_id": None},
            ]),
            product("p5", 20, 27, 30, "Standard", "Stock Base", [
                {"id": "p5_sb", "cost_category": "stock_base",
                 "cost_per_unit": 40, "multiplier_type": "per_base",
                 "units_per_product": 1, "tag_id": None},
            ]),
            product("p6", 1, 36, 60, "Standard", "Custom Base", [
                # Table 6: custom base unit costs
                {"id": "p6_uc1", "cost_category": "unit_cost",
                 "cost_per_unit": 200, "multiplier_type": "fixed",
                 "units_per_product": 1, "tag_id": None},
                {"id": "p6_uc2", "cost_category": "unit_cost",
                 "cost_per_unit": 50, "multiplier_type": "fixed",
                 "units_per_product": 1, "tag_id": None},
            ]),
        ]
        ...
        return {
            ...
            "group_cost_pools": [
                ...
                {
                    "id": "gp_ship", "total_amount": 25,
                    "distribution_type": "units", "cost_category": "stock_base_shipping",
                    "on_qty_change": "redistribute", "tag_id": None,
                    "members": [{"product_id": "p1"}, {"product_id": "p2"},
                                {"product_id": "p3"}, {"product_id": "p4"},
                                {"product_id": "p5"}],
                },
                # Misc: $500 by sqft, all 6 tables
                {
                    "id": "gp_misc", "total_amount": 500,
                    "distribution_type": "sqft", "cost_category": "misc",
                    "on_qty_change": "redistribute", "tag_id": None,
                    "members": [{"product_id": "p1"}, {"product_id": "p2"},
                                {"product_id": "p3"}, {"product_id": "p4"},
                                {"product_id": "p5"}, {"product_id": "p6"}],
                },
                # Consumables: $300 by sqft, all 6 tables
                {
                    "id": "gp_cons", "total_amount": 300,
                    "distribution_type": "sqft", "cost_category": "consumables",
                    "on_qty_change": "redistribute", "tag_id": None,
                    "members": [{"product_id": "p1"}, {"product_id": "p2"},
                                {"product_id": "p3"}, {"product_id": "p4"},
                                {"product_id": "p5"}, {"product_id": "p6"}],
    ...
    def test_species_cost_blocks(self):
        ...
        def species_cost(p):
            return next(b["cost_pp"] for b in p["cost_blocks"] if b["cost_category"] == "species")

        assert abs(species_cost(prods["p1"]) - Decimal("236.71")) < Decimal("0.01")
        assert abs(species_cost(prods["p2"]) - Decimal("617.50")) < Decimal("0.01")
        assert abs(species_cost(prods["p3"]) - Decimal("222.30")) < Decimal("0.01")
        assert abs(species_cost(prods["p4"]) - Decimal("247.00")) < Decimal("0.01")
        assert abs(species_cost(prods["p5"]) - Decimal("138.94")) < Decimal("0.01")
        ...
        assert abs(species_cost(prods["p6"]) - Decimal("370.50")) < Decimal("0.01")
    ...
    def test_stock_base_costs(self):
        ...
        def base_cost(p):
            return next((b["cost_pp"] for b in p["cost_blocks"] if b["cost_category"] == "stock_base"), None)

        assert base_cost(prods["p1"]) == Decimal("75.0000")
        assert base_cost(prods["p2"]) == Decimal("160.0000")
        assert base_cost(prods["p3"]) == Decimal("45.0000")
        assert base_cost(prods["p4"]) == Decimal("70.0000")  # 35 × 2 bases
        assert base_cost(prods["p5"]) == Decimal("40.0000")
        assert base_cost(prods["p6"]) is None  # custom base, no stock base block

    def test_sb_shipping_pool_by_units(self):
        """SB Shipping $25 across 33 units (Tables 1-5). Each gets $0.7576 PP."""
        result = compute_quote(self._build_quote())
        ship_pool = next(p for p in result["group_cost_pools"] if p["id"] == "gp_ship")
        members = {m["product_id"]: m for m in ship_pool["members"]}
    ...
    def test_misc_pool_uses_sq_ft_wl(self):
        ...
        result = compute_quote(self._build_quote())
        misc_pool = next(p for p in result["group_cost_pools"] if p["id"] == "gp_misc")
        members = {m["product_id"]: m for m in misc_pool["members"]}
    ...
    def test_rate_labor_lc101_hours(self):
        ...
        # Add LC101 rate labor block to each product
        for prod in quote_data["options"][0]["products"]:
            prod["labor_blocks"].append({
                "block_type": "rate",
                "labor_center": "LC101",
                "rate_value": 15,
                "metric_source": "panel_sqft",
                "is_active": True,
            })
        ...
        def lc101(p):
            return next(b["hours_pp"] for b in p["labor_blocks"] if b.get("labor_center") == "LC101")

        # Table 1: 9.5833 / 15 = 0.6389
        assert abs(lc101(prods["p1"]) - Decimal("0.6389")) < Decimal("0.001")
        # Table 2 (DIA): uses sq_ft_wl = 25, so 25 / 15 = 1.6667
        assert abs(lc101(prods["p2"]) - Decimal("1.6667")) < Decimal("0.001")
        # Table 3: 9.0 / 15 = 0.6000
        assert abs(lc101(prods["p3"]) - Decimal("0.6000")) < Decimal("0.001")
        # Table 5: 5.625 / 15 = 0.3750
        assert abs(lc101(prods["p5"]) - Decimal("0.3750")) < Decimal("0.001")
    ...
    def test_rate_labor_lc103_cutting(self):
        ...
        for prod in quote_data["options"][0]["products"]:
            prod["labor_blocks"].append({
                ...
                "labor_center": "LC103",
                "rate_value": 8,
                "metric_source": "panel_count",
                "is_active": True,
            })

        result = compute_quote(quote_data)
        for prod in result["options"][0]["products"]:
            lc103 = next(b["hours_pp"] for b in prod["labor_blocks"] if b.get("labor_center") == "LC103")
            # Every hardwood top is 1 panel → 1/8 = 0.125
            assert lc103 == Decimal("0.1250"), f"Product {prod['id']} LC103 hours_pp = {lc103}"
    ...
    def test_lc104_cnc_rate_type_units(self):
        ...
        def lc104(p):
            return next(
                b["hours_pp"] for b in p["labor_blocks"]
                if b.get("labor_center") == "LC104"
            )
        ...
        assert abs(lc104(prods["p2"]) - expected_t2) < Decimal("0.001"), \
            f"T2 LC104 hours_pp: expected ≈{expected_t2:.4f}, got {lc104(prods['p2'])}"
        assert abs(lc104(prods["p6"]) - expected_t6) < Decimal("0.001"), \
            f"T6 LC104 hours_pp: expected ≈{expected_t6:.4f}, got {lc104(prods['p6'])}"

        # Total hours = T2_hours_pt + T6_hours_pt (both qty=1) = 1.0
        total_lc104 = lc104(prods["p2"]) + lc104(prods["p6"])
        assert abs(total_lc104 - Decimal("1.0")) < Decimal("0.001"), \
            f"LC104 total hours should be 1.0, got {total_lc104}"

        # Tables 1/3/4/5 must have NO LC104 block
        for pid in ("p1", "p3", "p4", "p5"):
            assert not any(b.get("labor_center") == "LC104" for b in prods[pid]["labor_blocks"])
    ...
    def test_lc100_group_labor_pool_by_sqft(self):
        ...
        for prod in quote_data["options"][0]["products"]:
            if prod["id"] == "p6":
                prod["labor_blocks"].append({
                    "block_type": "unit", "labor_center": "LC100",
                    "hours_per_unit": "0.5", "is_active": True,
                })

        quote_data["group_labor_pools"] = [{
            "id": "glp_lc100",
            "labor_center": "LC100",
            "total_hours": "1.0",
            ...
            "members": [
                ...
                {"product_id": "p4"}, {"product_id": "p5"}, {"product_id": "p6"},
            ],
        }]

        result = compute_quote(quote_data)
        pool = result["group_labor_pools"][0]
        members = {m["product_id"]: m for m in pool["members"]}
        ...
        assert abs(members["p1"]["hours_pp"] - t1_expected) < Decimal("0.00001")

        # Pool total distributed = 1.0h
        total_dist = sum(members[pid]["hours_pt"] for pid in ("p1","p2","p3","p4","p5","p6"))
        assert abs(total_dist - Decimal("1.0")) < Decimal("0.001")

        # T6 also has 0.5h unit block on LC100
        prods = {p["id"]: p for p in result["options"][0]["products"]}
        t6_unit = next(b for b in prods["p6"]["labor_blocks"] if b.get("labor_center") == "LC100")
        assert t6_unit["hours_pp"] == Decimal("0.5")
    ...
    def test_lc105_unit_block_t6_only(self):
        ...
        lc105_t6 = next(b for b in prods["p6"]["labor_blocks"] if b.get("labor_center") == "LC105")
        assert lc105_t6["hours_pp"] == Decimal("3.5")
        assert lc105_t6["hours_pt"] == Decimal("3.5")  # qty=1

        # Other tables have no LC105
        for pid in ("p1", "p2", "p3", "p4", "p5"):
            assert not any(b.get("labor_center") == "LC105" for b in prods[pid]["labor_blocks"])
    ...
    def test_lc111_group_pool_plus_t6_unit(self):
        ...
        for prod in quote_data["options"][0]["products"]:
            if prod["id"] == "p6":
                prod["labor_blocks"].append({
                    "block_type": "unit", "labor_center": "LC111",
        ...
        pool_total = sum(members[pid]["hours_pt"] for pid in ("p1","p2","p3","p4","p5","p6"))
        assert abs(pool_total - Decimal("2.0")) < Decimal("0.001")

        # T6 unit block adds 0.7h separate from pool share
        t6_unit = next(b for b in prods["p6"]["labor_blocks"] if b.get("labor_center") == "LC111")
        assert t6_unit["hours_pp"] == Decimal("0.7")
    ...
    def _build_full_labor_quote(self):
        ...
        rate_all = [
            {"block_type": "rate", "labor_center": "LC101", "rate_value": 15,
             "metric_source": "panel_sqft", "is_active": True},
            {"block_type": "rate", "labor_center": "LC102", "rate_value": 40,
             "metric_source": "panel_sqft", "is_active": True},
            {"block_type": "rate", "labor_center": "LC103", "rate_value": 8,
             "metric_source": "panel_count", "is_active": True},
            {"block_type": "rate", "labor_center": "LC106", "rate_value": 12,
             "metric_source": "panel_sqft", "is_active": True},
            {"block_type": "rate", "labor_center": "LC109", "rate_value": 40,
             "metric_source": "panel_sqft", "is_active": True},
        ]
        lc104_block = {"block_type": "rate", "labor_center": "LC104",
                       "rate_value": 2, "metric_source": "sq_ft",
                       "rate_type": "units", "is_active": True}

        for prod in quote_data["options"][0]["products"]:
            for b in rate_all:
                prod["labor_blocks"].append(dict(b))
            if prod["id"] in ("p2", "p6"):
                prod["labor_blocks"].append(dict(lc104_block))
            if prod["id"] == "p6":
                prod["labor_blocks"].append({
                    "block_type": "unit", "labor_center": "LC100",
                    "hours_per_unit": "0.5", "is_active": True,
                ...
                prod["labor_blocks"].append({
                    "block_type": "unit", "labor_center": "LC105",
        ...
        quote_data["group_labor_pools"] = [
            {
                "id": "glp_lc100", "labor_center": "LC100",
                "total_hours": "1.0", "distribution_type": "sqft", "tag_id": None,
                "members": [{"product_id": p} for p in ("p1","p2","p3","p4","p5","p6")],
            },
            {
                "id": "glp_lc111", "labor_center": "LC111",
    ...
    def test_t6_unit_labor_hours(self):
        ...
        def hrs(lc):
            return next((b["hours_pp"] for b in p6["labor_blocks"] if b.get("labor_center") == lc), None)
        ...
        assert abs(hrs("LC104") - expected_lc104) < Decimal("0.001")

        # LC105: unit block 3.5h
        assert hrs("LC105") == Decimal("3.5")

        # LC111 unit block 0.7h (separate from group pool share)
        assert hrs("LC111") == Decimal("0.7")
...
class TestStonePipeline:
                ...
                "products": [
                    {
                        ...
                        "consumables_margin_rate": "0.00",
                        "cost_blocks": [
                            # Built-in stone block: $100/sqft via per_sqft
                            {"id": "b1", "cost_category": "stone",
    ...
    def test_stone_per_sqft_cost_block(self):
        ...
        p1_stone_pp = prods["p1"]["cost_blocks"][0]["cost_pp"]
        p2_stone_pp = prods["p2"]["cost_blocks"][0]["cost_pp"]
